...
do
  ...
  program_exit_code=$(<"${test_path}/${EXIT_CODE_FILE}")
  program_stdout=$(<"${test_path}/${PROGRAM_STDOUT_FILE}")
  
  source_file_path="${test_path}/main.c"
  assembly_file_path="${test_path}/main.s"
  out_file_path="${test_path}/out"

  ./hcc $source_file_path >/dev/null 2>&1
  actual_compiler_exit_code=$?
  if [[ ($compiler_exit_code -eq 0 && $actual_compiler_exit_code -ne 0) ||
        ($compiler_exit_code -ne 0 && $actual_compiler_exit_code -eq 0) ]]
  then
    echo "${test_path}: Unexpected compiler exit status"
    continue
  fi

  if [[ $compiler_exit_code -ne 0 ]]
  then
    continue
  fi

  ./assemble $assembly_file_path $out_file_path >/dev/null 2>&1
  if [[ $? -ne "0" ]]
  then
  ...
  if [[ $actual_program_stdout != $program_stdout ]]
  then
  ...
  if [[ $actual_program_exit_code -ne $program_exit_code ]]
  then
